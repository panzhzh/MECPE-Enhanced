...
Generates all possible emotion-cause pairs from Conv predictions
"""
import os
import torch
...
class PairDataset(Dataset):
    """
    Step2 dataset that generates emotion-cause pairs from Conv results
    Following the original TensorFlow data loading logic
    """
    
    def __init__(self, 
                 conv_result_file: str,
                 config,
                 tokenizer: Optional[RobertaTokenizer] = None,
                 split: str = "train"):
        """
        Args:
            conv_result_file: Path to Conv output file with emotion/cause predictions
        ...
        self.pairs, self.pair_id_all, self.pair_id = self._load_step2_data(conv_result_file)
    ...
    def _load_step2_data(self, conv_file: str) -> Tuple[List[Dict], List[Tuple], List[Tuple]]:
        """
        Load Step2 data from Conv results following original logic
        """
        pairs = []
        pair_id_all = []  # All true emotion-cause pairs
        pair_id = []      # All candidate pairs (for evaluation)
        
        print(f"Loading Step2 data from: {conv_file}")
...
def create_pair_datasets(config, tokenizer: Optional[RobertaTokenizer] = None):
    """
    Create Step2 datasets from Conv results
    ...
    """
    # Conv result files (generated by Conv training)
    conv_result_dir = os.path.join(config.training.save_dir, "conv_results")
    
    train_file = os.path.join(conv_result_dir, "train_predictions.txt")
    dev_file = os.path.join(conv_result_dir, "dev_predictions.txt") 
    test_file = os.path.join(conv_result_dir, "test_predictions.txt")
    
    # Check if Conv results exist
    for file_path, split in [(train_file, "train"), (dev_file, "dev"), (test_file, "test")]:
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"Conv result file not found: {file_path}. Please run Conv training first.")
